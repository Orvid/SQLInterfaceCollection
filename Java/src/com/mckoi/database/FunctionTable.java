...
public class FunctionTable extends DefaultDataTable {
  ...
  public void setWholeTableAsGroup() {
    ...
    // table.
    RowEnumeration senum = getReferenceTable().rowEnumeration();
    whole_table_is_simple_enum = senum instanceof SimpleRowEnumeration;
    if (!whole_table_is_simple_enum) {
      whole_table_group = new IntegerVector(getReferenceTable().getRowCount());
      while (senum.hasMoreRows()) {
        whole_table_group.addInt(senum.nextRowIndex());
      }
    }
  ...
  public Table mergeWithReference(Variable max_column) {
    ...
    if (whole_table_as_group) {
      ...
      RowEnumeration senum = table.rowEnumeration();
      while (senum.hasMoreRows()) {
        row_list.addInt(senum.nextRowIndex());
      }
    }
